...
def titania_import(tiedoston_nimi):
    ...
    pvm = str(pvm.copy().iloc[2, 9]).strip()[:8]
...
if __name__ == "__main__":
    df1, pvm = titania_import(r"C:\Users\pampi\Merikarhu\TAP01083.txt")
    # print(df1)
    # tt1 = csv_fix(df1)
    # print(tt1)
    # df2 = titania_import(r"C:\Users\pampi\Merikarhu\TPK12083.txt")
    # tt2 = csv_fix(df2)

    # tyontekijat = yhdista_tyontekijat([tt1, tt2])
    # ryhmat_omat, ryhmat_kaikki, lapset, tt_asetukset = ex.excel_import(
    #     r"C:\Users\pampi\Merikarhu\input\Merikarhu_input.csv"
    # )
    # for lapsi in lapset:
    #     print(lapsi)
    # tyontekijat = tt_asetukset_synkka(tyontekijat, tt_asetukset)

    # df = pd.DataFrame(tyontekijat)
    # df = df.iloc[:, 0:2]

    # df.to_csv("Merikarhu_output.csv", index=False)
